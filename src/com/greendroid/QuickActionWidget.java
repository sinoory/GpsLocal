...
public abstract class QuickActionWidget extends PopupWindow {
    ...
    public boolean changeQuickAction(Context ctx,int position,int drawableId, int titleId){
        if (position>=mQuickActions.size()){
            return false;
        }
        QuickAction qa=mQuickActions.get(position);
		qa.mDrawable= ctx.getResources().getDrawable(drawableId);
		qa.mTitle = ctx.getResources().getString(titleId);
		mIsDirty = true;
        return true;
    }
